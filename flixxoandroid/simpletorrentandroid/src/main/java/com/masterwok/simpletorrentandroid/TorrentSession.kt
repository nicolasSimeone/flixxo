...
open class TorrentSession(
    ...
    private fun isValidMagnetUri(magnetUrl:String): Boolean {

        val data = arrayOf(Byte)

        try {
            val data = sessionManager.fetchMagnet(magnetUrl, 3000)
    ...
    private fun setInitialTorrentState(torrentHandle: TorrentHandle) {
        ...
        val piecesCount = priorities.size
        if (piecesCount <= 0) return

        val startCount = Math.round(piecesCount * 0.15f) //as Int // first 25%
        val endCount = Math.round(piecesCount * 0.10f) //as Int // last 25%

        for (i in 0 until startCount) {
            priorities[i] = Priority.SEVEN
            Log.d(Tag, "torrent piece priority:  $i")
            torrentHandle.setPieceDeadline(i, 1)
        }

        for (i in (piecesCount-1) downTo (piecesCount - endCount)){
            priorities[i] = Priority.SIX
            Log.d(Tag, "torrent piece priority:  $i")
            torrentHandle.setPieceDeadline(i, 2)
        }
        ...
        torrentSessionBuffer = TorrentSessionBuffer(
            ...
            , torrentHandle.getStartPieceIndex(0)
    ...
    private fun onTorrentError(torrentErrorAlert: TorrentErrorAlert) {
        ...
        val errorValue = torrentHandle.status().errorCode().value()
        val errorMessage = torrentHandle.status().errorCode().message()

        Log.i("TorrentSession", "error :$errorValue message: $errorMessage" )
     ...
     */
    fun start(bencode: ByteArray, seed: String) {
        ...
        Log.i("TorrentSession", "seed: $seed")
        /*****FLIXXO EXTENSION*****/
        sessionManager.swig().add_bt_flixxo_extension(seed,"0.1.0")
        /*****FLIXXO EXTENSION*****/
     ...
     */
    fun start(context: Context, torrentUri: Uri, seed: String) {
        setInitialStartState()
